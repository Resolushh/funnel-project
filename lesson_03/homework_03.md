...
| Какое значение источника показал GA4 | (direct) / (none) |
...
Передача UTM-меток в аналитику не удалась. Тестовые переходы по ссылке стабильно определяются в GA4 как `(direct) / (none)`. 

Причина: встроенная функция предотвращения отслеживания (Tracking prevention) в браузере Microsoft Edge. В режиме InPrivate браузер использует жесткий профиль приватности, который блокирует маркетинговые скрипты Google Analytics на стороне клиента. GA4 не может считать параметры `utm_source` из адресной строки, хотя сама ссылка при загрузке страницы не обрезается и параметры в ней остаются.
